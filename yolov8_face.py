...
class YOLOv8_face:
    def __init__(self, path, conf_thres=0.2, iou_thres=0.5):
        ...
        self.feats_hw = [
            (math.ceil(self.input_height / stride), math.ceil(self.input_width / stride))
    ...
    def detect(self, srcimg):
        ...
        outputs = self.model.run(None, {'images': blob})
    ...
    def draw_detections(self, image, boxes, scores, kpts, roi=(0, 0)):
        roi_x, roi_y = roi
        for box, kp in zip(boxes, kpts):
            x1, y1, w, h = box.astype(int)
            # Adjust coordinates based on ROI
            x1 += roi_x
            y1 += roi_y
            # Draw rectangle around face
            cv2.rectangle(image, (x1, y1), (x1 + w, y1 + h), (0, 0, 255), thickness=3)
            # Draw keypoints
            for i in range(5):
                kp_x = int(kp[i * 3] + roi_x)
                kp_y = int(kp[i * 3 + 1] + roi_y)
                cv2.circle(image, (kp_x, kp_y), 4, (0, 255, 0), thickness=-1)
        return image
