...
def add_persons(backup_dir, add_persons_dir, faces_save_dir, features_path):
        ...
        for image_name in os.listdir(person_image_path):
            ...
            if image_name.endswith(("png", "jpg", "jpeg")):
                ...
                for i in range(len(bboxes)):
                    ...
                    # Get the location of the face, converting coordinates to integers
                    ...
                    if x1 < 0 or y1 < 0 or x2 > input_image.shape[1] or y2 > input_image.shape[0]:
                        print(f"Skipping face at ({x1}, {y1}, {x2}, {y2}) - bounding box out of bounds.")
                        continue

                    # Extract the face from the image
                    face_image = input_image[y1:y2, x1:x2]

                    # Check if face_image is empty and skip if it is
    ...
    for sub_dir in os.listdir(add_persons_dir):
        ...
        destination_path = os.path.join(backup_dir, sub_dir)

        if os.path.exists(destination_path):
            shutil.rmtree(destination_path)
            print(f"Existing backup found. Removed old backup: {destination_path}")

        shutil.move(dir_to_move, backup_dir, copy_function=shutil.copytree)
        print(f"Successfully moved {dir_to_move} to {backup_dir}")
...
if __name__ == "__main__":
    ...
    os.system('find ./datasets -name ".DS_Store" -delete')
